...
func TestErrorHandler_ClientCanceled_DirectErrorIs(t *testing.T) {
	gin.SetMode(gin.TestMode)
	log := &testLogger{}
	...
	r.Use(ErrorHandler(log))
	r.GET("/test", func(c *gin.Context) {
		// Una capa interna devolvió un error envuelto sobre context.Canceled.
		_ = c.Error(fmt.Errorf("repo failed: %w", context.Canceled)) //nolint:errcheck
	})
	...
	// Body vacío: 499 no devuelve payload (cliente ya cerró la conexión).
	assert.Empty(t, w.Body.String())
}

func TestErrorHandler_ClientCanceled_RequestContextCanceled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	log := &testLogger{}
	...
	r.Use(ErrorHandler(log))
	r.GET("/test", func(c *gin.Context) {
		// Simulación: capa inferior reportó un error genérico, pero el
		// contexto del request fue cancelado por el cliente.
		ctx, cancel := context.WithCancel(c.Request.Context())
		cancel()
		c.Request = c.Request.WithContext(ctx)
		_ = c.Error(stderrors.New("query failed")) //nolint:errcheck
	})
...
func TestErrorHandler_DeadlineExceededIsNotClientCanceled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	log := &testLogger{}
	...
	r.Use(ErrorHandler(log))
	r.GET("/test", func(c *gin.Context) {
		_ = c.Error(fmt.Errorf("timeout: %w", context.DeadlineExceeded)) //nolint:errcheck
	})
	...
	// Timeout del servidor: sigue siendo 500, no 499.
	assert.Equal(t, http.StatusInternalServerError, w.Code)
}
