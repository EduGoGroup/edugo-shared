	...
	t.Run("permite acceso con permiso correcto", func(t *testing.T) {
		...
		claims := &auth.Claims{
			...
			ActiveContext: &auth.UserContext{
				RoleID:   "role-teacher",
				RoleName: "Teacher",
				Grants:   auth.Grants{Allow: []string{"admin.users.read", "content.materials.create"}},
			},
		}
	...
	t.Run("permite acceso cuando el usuario tiene el permiso entre varios", func(t *testing.T) {
		c := createTestContextWithClaims(&auth.Claims{
			...
			ActiveContext: &auth.UserContext{
				RoleID:   "role-admin",
				RoleName: "Admin",
				Grants: auth.Grants{Allow: []string{
					"admin.users.create",
					"admin.users.read",
					"admin.users.update",
					"admin.users.delete",
	...
	t.Run("permite acceso con uno de los permisos requeridos", func(t *testing.T) {
		c := createTestContextWithClaims(&auth.Claims{
			...
			ActiveContext: &auth.UserContext{
				RoleID:   "role-teacher",
				RoleName: "Teacher",
				Grants:   auth.Grants{Allow: []string{"content.materials.read", "content.materials.create"}},
			},
		})
	...
	t.Run("permite acceso con el último permiso de la lista", func(t *testing.T) {
		c := createTestContextWithClaims(&auth.Claims{
			...
			ActiveContext: &auth.UserContext{
				RoleID:   "role-teacher",
				RoleName: "Teacher",
				Grants:   auth.Grants{Allow: []string{"admin.users.delete"}},
			},
		})
	...
	t.Run("rechaza acceso si falta un permiso", func(t *testing.T) {
		c := createTestContextWithClaims(&auth.Claims{
			...
			ActiveContext: &auth.UserContext{
				RoleID:   "role-teacher",
				RoleName: "Teacher",
				Grants:   auth.Grants{Allow: []string{"admin.users.read", "admin.users.update"}},
			},
		})
...
func TestPermissionMiddleware_Integration(t *testing.T) {
	...
	t.Run("integración completa con token JWT real", func(t *testing.T) {
		...
		activeContext := &auth.UserContext{
			...
			Grants:     auth.Grants{Allow: []string{"content.materials.create", "content.materials.read", "content.materials.update"}},
		}
	...
	t.Run("integración rechaza acceso sin permiso", func(t *testing.T) {
		...
		activeContext := &auth.UserContext{
			RoleID:   "role-student",
			RoleName: "Student",
			Grants:   auth.Grants{Allow: []string{"content.materials.read"}}, // Solo lectura
		}
