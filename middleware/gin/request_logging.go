...
func RequestLogging(baseLogger logger.Logger) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// trace_id/span_id se extraen del span activo (inyectado por
		// tracer.GinMiddleware si está montado antes que este middleware)
		// para correlacionar logs con trazas en el backend de observabilidad.
		fields := []any{
		...
		if sc := trace.SpanContextFromContext(c.Request.Context()); sc.IsValid() {
			fields = append(fields,
				logger.FieldTraceID, sc.TraceID().String(),
				logger.FieldSpanID, sc.SpanID().String(),
			)
		}
		...
		// Si el cliente canceló la conexión, reclasificamos a 499 para
		// observabilidad aunque el handler ya haya escrito otro status
		// (típicamente 500 en handlers que no pasan por HandleError).
		// El cliente nunca verá este código; sólo afecta logs/métricas.
		clientCanceled := c.Request != nil && c.Request.Context() != nil &&
			stderrors.Is(c.Request.Context().Err(), context.Canceled)
		if clientCanceled {
		...
		if clientCanceled {
			attrs = append(attrs, "client_canceled", true)
		}
		...
		case status == StatusClientClosedRequest:
			// 499 (cliente canceló): no es error de aplicación.
