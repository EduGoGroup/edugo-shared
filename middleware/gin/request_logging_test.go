...
func TestRequestLogging_ReclassifiesCanceledContextAs499(t *testing.T) {
	// Cubre el caso del handler legacy que escribe 500 directo cuando el
	// cliente ya canceló la conexión (ej. /sync/bundle de platform).
	// El log debe reportar 499/Info, no 500/Error.
	buf := &bytes.Buffer{}
	r, _ := setupLoggingTestRouter(buf)
	...
	require.NoError(t, json.Unmarshal(buf.Bytes(), &entry))
	assert.Equal(t, "INFO", entry["level"])
	assert.Equal(t, float64(StatusClientClosedRequest), entry["status_code"])
	assert.Equal(t, true, entry["client_canceled"])
}

func TestRequestLogging_LogsInfoFor499(t *testing.T) {
	buf := &bytes.Buffer{}
	r, _ := setupLoggingTestRouter(buf)
	r.GET("/test", func(c *gin.Context) { c.Status(StatusClientClosedRequest) })
